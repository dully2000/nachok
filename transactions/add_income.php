...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($csrfToken)) {
        ...
        $error = "Please enter a valid positive income amount.";
    } elseif ($categoryId <= 0) {
        ...
        try {
            $stmt = $db->prepare("
                ...
                VALUES (:uid, :cat, 'income', :amt, :desc, :tdate)
            ");
            $stmt->execute([
                ':uid' => $userId,
                ':cat' => $categoryId,
            ...
            logActivity("Added Income: " . formatCurrency($amount));
            setFlash('success', 'Income transaction recorded successfully.');
...
$catStmt = $db->prepare("SELECT * FROM categories WHERE (user_id IS NULL OR user_id = :uid) AND type = 'income' ORDER BY name ASC");
$catStmt->execute([':uid' => $userId]);
$categories = $catStmt->fetchAll();

$pageTitle = "Add Income - CODE X";
    ...
    <div class="max-w-xl mx-auto">
        <div class="codex-card p-4 p-md-5">
            <div class="d-flex align-items-center mb-4">
                <div class="stat-icon stat-icon-income me-3">
                    <i class="fa-solid fa-arrow-down-left"></i>
                </div>
                <div>
                    <h2 class="h4 font-heading text-light mb-0">Record Income</h2>
                    <p class="small text-secondary mb-0">Add salary, freelance, business, or gift earnings</p>
                </div>
            </div>
            ...
            <form action="add_income.php" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" min="0.01" name="amount" class="form-control form-control-lg text-success fw-bold" placeholder="0.00" required>
                    </div>
                </div>

                <div class="mb-3">
                    <label class="form-label text-light small fw-medium">Income Category</label>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="form-control" rows="3" placeholder="e.g., Monthly salary payment or Project milestone"></textarea>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <button type="submit" class="btn btn-success px-4 py-2 fw-semibold">
                        <i class="fa-solid fa-check me-1"></i> Save Income
                    </button>
                </div>
            </form>
        </div>
